terraform {
  backend "s3" {
    ...
    region = "us-east-2"
    key    = "lambdas/hub-payment-card/dev"
  }
}
...
resource "aws_lambda_function" "my_lambda" {
  ...
  environment {
    variables = {
      MERCADO_PAGO_CARD          = "https://api.mercadopago.com/v1/customers/{customer_id}/cards"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:dynamodb:${var.aws_region}:${var.aws_account}:table/hub-payment-cards-${var.aws_env}"
      },
      {
        Effect = "Allow"
        Action = [
