terraform {
  backend "s3" {
    ...
    region = "us-east-2"
    key    = "lambdas/hub-payment-order-payment/dev"
  }
}
...
resource "aws_lambda_function" "my_lambda" {
  ...
  environment {
    variables = {
      AWS_ENV                  = var.aws_env
      MERCADO_PAGO_GET_PAYMENT = var.env["MERCADO_PAGO_GET_PAYMENT"]
    }
  }
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:dynamodb:${var.aws_region}:${var.aws_account}:table/hub-payment-scheduler-status-${var.aws_env}"
      }
    ]
  })
}
